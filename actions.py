...
def miner_to_ore(world, entity, ore):
   ...
   if adjacent(entity_pt, ore_pt):
      entities.entity.set_resource_count(
         1 + entities.entity.get_resource_count())
...
def miner_to_smith(world, entity, smith):
   ...
   if adjacent(entity_pt, smith_pt):
      entities.smith.set_resource_count(
         ...
         entities.entity.get_resource_count())
...
def create_miner_not_full_action(world, entity, i_store):
   def action(current_ticks):
      entities.entity.remove_pending_action(action)
...
def blob_to_vein(world, entity, vein):
   ...
   if not vein:
      return ([entity_pt], False)
   vein_pt = entities.vein.get_position()
...
def create_ore_blob_action(world, entity, i_store):
   def action(current_ticks):
      ...
      next_time = current_ticks + entities.entity.get_rate()
      if found:
         quake = create_quake(world, tiles[0], current_ticks, i_store)
         worldmodel.add_entity(world, quake)
         next_time = current_ticks + entities.entity.get_rate() * 2
...
def create_vein_action(world, entity, i_store):
   def action(current_ticks):
      ...
      open_pt = find_open_around(world, entities.entity.get_position(),
         entities.entity.get_resource_distance())
      if open_pt:
         ore = create_ore(world,
            "ore - " + entities.entity.get_name() + " - " + str(current_ticks),
      ...
      schedule_action(world, entity,
         ...
         current_ticks + entities.entity.get_rate())
      return tiles
   return action
...
def try_transform_miner_full(world, entity):
   new_entity = entities.MinerNotFull(
      entities.entity.get_name(), entities.entity.get_resource_limit(),
      entities.entity.get_position(), entities.entity.get_rate(),
      entities.entity.get_images(), entities.entity.get_animation_rate())
...
def try_transform_miner(world, entity, transform):
   ...
   if entity != new_entity:
      ...
      worldmodel.remove_entity_at(world, entities.entity.get_position())
...
def create_animation_action(world, entity, repeat_count):
   def action(current_ticks):
      ...
      entities.entity.next_image()

      if repeat_count != 1:
         schedule_action(world, entity,
            create_animation_action(world, entity, max(repeat_count - 1, 0)),
            current_ticks + entities.entity.get_animation_rate())

      return [entities.entity.get_position()]
   return action
...
def create_entity_death_action(world, entity):
   def action(current_ticks):
      entities.entity.remove_pending_action(action)
      pt = entities.entity.get_position()
...
def create_ore_transform_action(world, entity, i_store):
   def action(current_ticks):
      entities.entity.remove_pending_action(action)
      blob = create_blob(world, entities.entity.get_name() + " -- blob",
         entities.entity.get_position(),
         entities.entity.get_rate() // BLOB_RATE_SCALE,
...
def remove_entity(world, entity):
   for action in entities.entity.get_pending_actions():
      worldmodel.unschedule_action(world, action)
   entities.entity.clear_pending_actions()
...
def schedule_vein(world, vein, ticks, i_store):
   schedule_action(world, vein, create_vein_action(world, vein, i_store),
      ticks + entities.vein.get_rate())


def schedule_action(world, entity, action, time):
   entities.entity.add_pending_action(action)
...
def schedule_animation(world, entity, repeat_count=0):
   schedule_action(world, entity,
      ...
      entities.entity.get_animation_rate())
